...
@app.route('/')
def home():
    tables = [{'name':'Tenants', 'link':'tenants'}, {'name':'Assets', 'link':'assets'}]
...
def settings():
    ...
    return render_template('/settings/index.html', title='Settings', parent_link='/settings', left_nav_bar_items=modules)


@app.route('/settings/<module>', methods=['GET', 'POST'])
